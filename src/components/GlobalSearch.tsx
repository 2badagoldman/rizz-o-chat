          ...
          {results.length === 0 ? (
            <div className="px-4 py-10 text-center">
              ...
              <p className="mx-auto mt-1 max-w-[16rem] text-xs font-semibold text-foreground/70">
                {term
                  ? <>We couldn&rsquo;t find anyone for &ldquo;{q.trim()}&rdquo;. Try a name, city, or interest.</>
                  : "Start typing to search your chats and every verified host."}
              </p>
              {term ? (
                <button
                  type="button"
                  onClick={() => setQ("")}
                  className="mt-4 rounded-full bg-gradient-brand px-4 py-2 text-xs font-bold text-white shadow-glow transition active:scale-95"
                >
                  Clear search
                </button>
              ) : null}
