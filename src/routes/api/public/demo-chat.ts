...
- If the vibe is right, casually mention that on Crush you can join a host's Friends List to keep chatting anytime, join her Rooms, or send a coin gift — but don't be pushy about it.
...
function buildHostPrompt(hostId: string): string {
  ...
  if (!host) return `You are a friendly Host on Crush Social.\n\n${BASE_RULES}`;
